...
import os, dotenv, pathlib, typing, hashlib
...
class User(db.Model, UserMixin):
    ...
    email: Mapped[str] = mapped_column(String, unique=True, nullable=False)
    password: Mapped[str] = mapped_column(String, unique=True, nullable=False)
    children_blog: Mapped[typing.List["BlogPost"]] = relationship(back_populates="parent_user")
    comment_blog: Mapped[typing.List["BlogComment"]] = relationship(back_populates="parent_user_b")
...
class BlogPost(db.Model, UserMixin):
    ...
    comments: Mapped[typing.List["BlogComment"]] = relationship(back_populates="parent_post")
...
def register():
    ...
    if form.validate_on_submit():
        ...
        hashed_psw = generate_password_hash(form.password.data, method="pbkdf2:sha256", salt_length=8)
        new_user = User(
...
def login():
    ...
    if form.validate_on_submit():
        ...
        existed_user = db.session.execute(db.select(User).where(User.email==email)).scalar()
        if existed_user:
            ...
            flash("That Email does not exist or incorrect, please try again or register")
...
def show_post(post_id):
    ...
    comments_data = db.session.execute(db.select(BlogComment).where(BlogComment.post_id==post_id)).scalars()
    all_comments = [comment for comment in comments_data]
    # all_avatar_links = [gravatar_url(com.parent_user_b.email) for com in all_comments]
    return render_template("post.html", post=requested_post, form=form, comments=all_comments, gravatar_url=gravatar_url, logged_in=login_checker())
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", form=edit_form, is_edit=True, logged_in=login_checker())
